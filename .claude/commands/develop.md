...
2. **Read content strategy docs** (ALWAYS read these):
   ...
   - `content/packaging-strategy.md` — title/thumbnail/hook frameworks (if it exists)
...
1. **3-5 hook lines** — The first 1-2 lines that appear before "see more." Each should stop the scroll. Follow the post framework in `reference/linkedin-post-framework.md` if available:
   - Open with a confrontational truth or bold claim
   - Include a specific number or metric
   - No hedging, no soft openings
...
1. **3-5 title options** — Each using a different hook element (curiosity, authority, list, timeliness)
2. **Thumbnail concepts** — Emotion, text overlay, visual description
3. **Hook script** — First 30 seconds that retain viewers
